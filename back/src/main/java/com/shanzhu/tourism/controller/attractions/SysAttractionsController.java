...
public class SysAttractionsController {
    ...
    public Result getSysAttractionsIndex(HttpServletRequest request) {
        ...
        // 尝试从请求头获取用户ID，这是前端直接传递的
        String headerUserId = request.getHeader("user_id");
        
        log.info("获取推荐景点请求，JWT用户ID: {}, 请求头用户ID: {}", userId, headerUserId);
        
        // 优先使用JWT中的用户ID，如果没有则使用请求头中的用户ID
        if (StringUtils.isBlank(userId) && StringUtils.isNotBlank(headerUserId)) {
            userId = headerUserId;
            log.info("使用请求头中的用户ID: {}", userId);
        }
        
        // 如果用户未登录，使用访客模式
        if (StringUtils.isBlank(userId)) {
            log.info("用户未登录，使用访客模式");
            userId = "visitor-" + request.getSession().getId();
        }
        
        // 调用服务获取推荐景点
        List<SysAttractions> recommendedAttractions = sysAttractionsService.searchAttractions(userId);
        log.info("为用户 {} 推荐了 {} 个景点", userId, recommendedAttractions.size());
        
        return Result.success(recommendedAttractions);
    }
