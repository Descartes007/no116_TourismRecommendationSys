<template>
<div>
  <el-dialog title="新增旅游线路" width="50%" :destroy-on-close="true" :visible.sync="addVisible" :before-close="handleClose" custom-class="line-dialog">
      ...
      submit(){
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            ...
            console.log('提交新增线路数据:', this.form);
            saveSysLine(this.form).then(res => {
              this.submitLoading = false;
              console.log('新增线路返回结果:', res);
              if(res.code == 1000) {
                this.$message({
                  type: 'success',
                  message: '保存成功!'
